...
const server = spawn(process.execPath, [".next/standalone/server.js"], {
  cwd: process.cwd(),
  env: { ...process.env, PORT: "3120", HOSTNAME: "127.0.0.1" },
...
function runSmoke() {
  return new Promise((resolve, reject) => {
    const smoke = spawn(process.execPath, ["--env-file=.env.local", "scripts/accounting-http-smoke.mjs"], {
      cwd: process.cwd(),
    ...
    smoke.once("exit", (code) => code === 0 ? resolve() : reject(new Error(`La prueba HTTP contable terminó con código ${code}`)));
  });
}
